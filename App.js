...
import {EMPTY_ARRAY, STORAGE_KEY} from "./utils";
...
export default function App() {
  ...
  async function loadTodoList() {
    try {
      const todoListString = await AsyncStorage.getItem(STORAGE_KEY);
      const parsedTodoList = JSON.parse(todoListString);
      isLoadUpdate = true;
      setTodoList(parsedTodoList || EMPTY_ARRAY);
    } catch (err) {
      alert(err);
    }
  }
